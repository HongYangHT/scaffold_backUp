(function() {
    var myModule = {
        lazyload: function() {
            setTimeout(function() {
                $('.YX-N-M-C1C5 .J_lazyload').lazyload({
                    threshold: 50,
                    effect: 'fadeIn',
                    opacity:true
                });
        ...
        getGoodsRender: function(element, id, subject,needScenePicUrl) {
            ...
            self.getAjaxData('https://spread.mail.163.com/mail/goods/getGroup', {
                ...
                $.each(_goods, function(k, v) {
                    ...
                    if ((v.status == 2) && !v.underShelf) {
                        _html += [
                            ...
                            '<a href="http://you.163.com/item/detail?id=' + v.id + '&&_stat_subject=' + _subject + '" target="_blank" class="u-C1C5-link">',
                            ...
                            '<span class="u-C1C5-offPrice">' + v.offPrice + '</span>',
                            (parseFloat(v.offPrice) == parseFloat(v.unitPrice) ? '' : '<span class="u-C1C5-unitPrice">' + v.unitPrice + '</span>'),
                            '</p>',
                            ...
                            '<a href="javascript:;" target="_self" class="u-C1C5-cart J_C1C5_cart" data-skuId="' + sellVolumeId + '" data-img="' + v.primaryPicUrl + '?imageView&thumbnail=260x260&quality=95"></a>',
                            '</div>',
                            '</div>',
                            '</div>'
                        ].join('');
                ...
                $(element).find('.m-C1C5-goods .u-C1C5-goods').remove()
    ...
    $('.YX-N-M-C1C5 .J_module_info ').each(function(i, n) {
        ...
        myModule.getGoodsRender($(n).closest('.YX-N-M-C1C5'), _id, window.psc_act_id,needScenePicUrl);
    });
